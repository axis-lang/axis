...
class SessionState(Builtin):
    ...
    local_facts: tuple[protomorph.Spec, ...] = ()
    deferred: tuple[urs.DeferredGoal, ...] = ()
    tables: urs.SessionTables = SessionTables()
    epoch: int = 0
    binding_epoch: int = 0
    local_facts_epoch: int = 0
    recent_binding_updates: tuple[protomorph.Placeholder, ...] = ()
    recent_local_fact_anchors: tuple[Anchor, ...] = ()
...
class Session(Consed):
    ...
    def query(self, goal: protomorph.Spec) -> urs.Query:
        return urs.Query(self, goal)

    def solve(self, goal: protomorph.Spec):
        return self.query(goal).result.outcome

    def with_bindings(
        self,
        bindings: frozendict[protomorph.Placeholder, urs.ReasoningValue],
    ) -> urs.Session:
    ...
    def with_deferred(self, deferred: tuple[urs.DeferredGoal, ...]) -> urs.Session:
        merged: dict[tuple[protomorph.Spec, object], urs.DeferredGoal] = {
    ...
    def with_query_table(self, key: protomorph.Spec, table: urs.QueryTable) -> urs.Session:
        ...
        for fact in _promoted_facts(table):
            answers_by_anchor.setdefault(fact.anchor, []).append(fact)
    ...
    def without_goal(self, key: protomorph.Spec) -> urs.Session:
        tables_by_goal = dict(self.state.tables.query_tables)
    ...
    def with_local_facts(self, *facts: protomorph.Spec) -> urs.Session:
        next_state = SessionState(
            self.state.bindings,
            ...
            (),
            tuple(dict.fromkeys(fact.anchor for fact in facts)),
        )
        return Session(self.engine, self.context, next_state)
...
def _promoted_facts(table: urs.QueryTable) -> tuple[protomorph.Spec, ...]:
...
def _dedupe_specs(values: list[protomorph.Spec]) -> list[protomorph.Spec]:
    deduped: dict[protomorph.Spec, protomorph.Spec] = {}
    for value in values:
        deduped[value] = value
...
def _build_session_tables(
    query_tables: dict[protomorph.Spec, urs.QueryTable],
    answers_by_anchor: frozendict[Anchor, tuple[protomorph.Spec, ...]],
) -> urs.SessionTables:
    deferred_by_anchor: dict[Anchor, set[protomorph.Spec]] = {}
    deferred_by_placeholder: dict[protomorph.Placeholder, set[protomorph.Spec]] = {}
