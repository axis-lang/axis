...
class _CompiledLiteral(protomorph.Builtin):
    goal: protomorph.Carrier
    negated: bool = False


class _CompiledRule(protomorph.Builtin):
    rule: urs.Rule
    ctx: urs.RuleCtx
    head: protomorph.Carrier
    body: tuple[_CompiledLiteral, ...]


class _AnswerData(protomorph.Builtin):
    subst: tuple[tuple[int, protomorph.Carrier], ...]
    evidence: protomorph.Spec | None = None
    judgment: urs.Judgment | None = None


class _GoalOutcome(protomorph.Builtin):
...
class _ResumeBranchResult(protomorph.Builtin):
    answers: tuple[_AnswerData, ...] = ()
    branches: tuple[urs.PendingBranch, ...] = ()
    cycle_issue: urs.CycleIssue | None = None
    failures: tuple[urs.Judgment, ...] = ()
...
class _GoalEntry:
    ...
    closed: bool = False
    coinductive: bool = False
    answers: dict[tuple[tuple[int, protomorph.Carrier], ...], _AnswerData] = field(default_factory=dict)
    deferred: dict[tuple[protomorph.Spec, object], DeferredGoal] = field(default_factory=dict)
    branches: dict[
        tuple[
            urs.DeferredGoal,
...
class _QueryCore:
    def __init__(self, session: urs.Session, goal: protomorph.Spec):
        self.session = session
        self.goal = goal
        self.engine = session.engine
        self._compiled_rules: dict[urs.Rule, _CompiledRule] = {}
        self._tables: dict[protomorph.Spec, _GoalEntry] = {}
        self._next_owner = 1
        self._active_stack: list[_ActiveFrame] = []

    def run(self) -> tuple[CanonicalGoal, tuple[protomorph.Placeholder, ...], _GoalOutcome]:
    ...
    def _solve_goal(self, goal: CanonicalGoal, *, via_negation: bool = False) -> _GoalOutcome:
        if self.engine.strata.has_negative_cycle(goal.key.anchor):
            trace = self.engine.strata.negative_cycle_trace(goal.key.anchor) or CycleTrace(
                (CycleMember(goal.key, self.engine.realm.is_coinductive_anchor(goal.key.anchor), via_negation),),
                "negative",
        ...
        coinductive = self.engine.realm.is_coinductive_anchor(goal.key.anchor)
        ...
        while True:
            ...
            for rule in self.engine.rules_for_anchor(goal.key.anchor):
    ...
    def _active_cycle(self, goal: protomorph.Spec, coinductive: bool, via_negation: bool = False) -> _GoalOutcome | None:
    ...
    def _cycle_trace(self, index: int, goal: protomorph.Spec, coinductive: bool, via_negation: bool) -> urs.CycleTrace:
    ...
    def _operator_outcome(self, goal: CanonicalGoal) -> _GoalOutcome:
        ...
        step = operator.eval(goal=goal.key, session=self.session, realm=self.engine.realm)
    ...
    def _expanded_outcome(self, goal: CanonicalGoal, goals: tuple[protomorph.Spec, ...]) -> _GoalOutcome:
        answers: list[_AnswerData] = []
    ...
    def _rule_outcome(self, goal: CanonicalGoal, rule: urs.Rule) -> _GoalOutcome:
        ...
        def solve_body(index: int, subjudgments: tuple[urs.Judgment, ...]) -> None:
            ...
            cycle = self._active_cycle(subgoal.key, self.engine.realm.is_coinductive_anchor(subgoal.key.anchor), literal.negated)
            if cycle is not None:
    ...
    def _negative_outcome(self, parent: CanonicalGoal, goal: CanonicalGoal) -> _GoalOutcome:
        ...
        if self.engine.strata.has_negative_cycle(target_anchor):
            trace = self.engine.strata.negative_cycle_trace(target_anchor) or CycleTrace(
                (CycleMember(goal.key, self.engine.realm.is_coinductive_anchor(target_anchor), True),),
                "negative",
                "negative cycle in stratification",
                True,
            )
        ...
        parent_stratum = self.engine.strata.stratum_of(parent.key.anchor)
    ...
    def _facts_for_anchor(self, anchor: Anchor) -> tuple[protomorph.Spec, ...]:
        local = tuple(fact for fact in self.session.state.local_facts if fact.anchor == anchor)
    ...
    def _instantiate_goal(self, goal: CanonicalGoal) -> protomorph.Carrier:
    ...
    def _first_operator(self, goal: protomorph.Spec) -> SolverOperator | None:
        return _search_operator(goal)

    def _from_public_answer(self, goal: CanonicalGoal, answer: protomorph.Builtin) -> _AnswerData:
    ...
    def _from_bound_answer(self, goal: CanonicalGoal, step: OpBind) -> _AnswerData:
        subst: list[tuple[int, protomorph.Carrier]] = []
    ...
    def _new_rule_app_ctx(self, parent_goal: protomorph.Spec, rule_ctx: urs.RuleCtx) -> RuleAppCtx:
    ...
    def _persist_runtime_branch(
        ...
        blocked_carrier: protomorph.Carrier,
        remaining_carriers: tuple[protomorph.Carrier, ...],
        uf: UnionFind,
...
class _SessionSolveCore:
    ...
    def _open_tables(self, session: urs.Session) -> tuple:
        candidates: dict[protomorph.Spec, object] = {}
    ...
    def _resume_table(self, table, session: urs.Session):
        ...
        answers_by_subst: dict[tuple[tuple[int, protomorph.Carrier], ...], urs.StoredAnswer] = {
    ...
    def _resume_branch_state(
        ...
        session: urs.Session,
        uf: UnionFind,
        current_carrier: protomorph.Carrier,
        remaining_carriers: tuple[protomorph.Carrier, ...],
                ...
                return _ResumeBranchResult(
                    branches=tuple(
                        self._persist_spec_branch(
                            inner,
                            trailing_goals,
                            cast(dict[protomorph.Placeholder, EqClassInfo | None], {**outer_info, **branch_placeholder_info(inner)}),
                            branch.completion,
    ...
    def _persist_spec_branch(
        self,
        branch: urs.PendingBranch,
        trailing_goals: tuple[protomorph.Spec, ...],
        info_by_placeholder: dict[protomorph.Placeholder, EqClassInfo | None],
        completion: urs.BranchCompletion | None,
        subjudgments: tuple[urs.Judgment, ...],
    ) -> urs.PendingBranch:
    ...
    def _wake_satisfied(self, table, wake, session: urs.Session) -> bool:
        ...
        if isinstance(wake, LocalFactsChanged):
            ...
            if not wake.anchors:
                return True
            return any(fact.anchor in wake.anchors for fact in session.state.local_facts)
...
class _EngineSolveCore:
    ...
    def run(self):
        known: dict[Anchor, set[protomorph.Spec]] = {
            anchor: set(self.engine.facts_by_anchor.get(anchor, ()))
            for anchor in self.engine.anchors
        }
        derived: dict[Anchor, set[protomorph.Spec]] = {anchor: set() for anchor in self.engine.anchors}
                    ...
                    for anchor in component.anchors:
                        for rule in self.engine.rules_by_anchor.get(anchor, ()):
                            for fact in _derive_ground_facts(rule, known, self.engine.strata, stratum):
                                bucket = known.setdefault(anchor, set())
                                if fact in bucket:
                                    continue
                                bucket.add(fact)
                                if fact not in self.engine.facts_by_anchor.get(anchor, ()):
                                    derived.setdefault(anchor, set()).add(fact)
                                changed = True
                if not changed:
                    break
            for component in components:
        ...
        facts_by_component: dict[int, tuple[protomorph.Spec, ...]] = {}
        derived_by_component: dict[int, tuple[protomorph.Spec, ...]] = {}
        for component in self.engine.sccs:
            if component.id not in closed_components:
                continue
            facts: list[protomorph.Spec] = []
            component_derived: list[protomorph.Spec] = []
            for anchor in component.anchors:
...
def _dedupe_deferred(items: list[DeferredGoal]) -> list[DeferredGoal]:
    deduped: dict[tuple[protomorph.Spec, object], DeferredGoal] = {}
    for item in items:
        deduped[(item.goal, item.blocker)] = item
    return list(deduped.values())


def _dedupe_specs(items: list[protomorph.Spec]) -> list[protomorph.Spec]:
    deduped: dict[protomorph.Spec, None] = {}
    for item in items:
        deduped[item] = None
    return list(deduped)
...
def _preferred_failures(outcome: _GoalOutcome, goal: protomorph.Spec) -> tuple[urs.Judgment, ...]:
    if outcome.failures:
        return outcome.failures
    return (_failure_judgment(goal, "no matching proof"),)


def _failure_judgment(goal: protomorph.Spec, reason: str, subjudgments: tuple[urs.Judgment, ...] = ()) -> urs.Judgment:
...
def _judgment_score(judgment: urs.Judgment | None) -> tuple[int, int]:
    if judgment is None or judgment.evidence is None:
        return (0, 0)
    anchor = judgment.evidence.anchor
    anchor_score = {
        "std.logic.ByRule": 4,
...
def _evidence_fact(fact: protomorph.Spec) -> protomorph.Spec:
    return protomorph.Spec.of("std.logic.ByFact", fact)


def _evidence_rule(rule: urs.Rule, subjudgments: tuple[urs.Judgment, ...] = ()) -> protomorph.Spec:
    return protomorph.Spec.of("std.logic.ByRule", rule.head, *subjudgments)


def _evidence_rule_head(rule_head: protomorph.Spec, subjudgments: tuple[urs.Judgment, ...] = ()) -> protomorph.Spec:
    return protomorph.Spec.of("std.logic.ByRule", rule_head, *subjudgments)


def _evidence_expand(goal: protomorph.Spec, subjudgments: tuple[urs.Judgment, ...] = ()) -> protomorph.Spec:
    return protomorph.Spec.of("std.logic.ByExpand", goal, *subjudgments)
...
def _evidence_coinduction(goal: protomorph.Spec) -> protomorph.Spec:
    return protomorph.Spec.of("std.logic.ByCoinduction", goal)


def _evidence_deferred(goal: protomorph.Spec, blocker) -> protomorph.Spec:
    return protomorph.Spec.of("std.logic.ByDeferred", goal, blocker)


def _evidence_negation(goal: protomorph.Spec) -> protomorph.Spec:
    return protomorph.Spec.of("std.logic.ByNegation", goal)


def _deferred_goal(goal: protomorph.Spec, blocker, evidence: protomorph.Spec | None = None) -> urs.DeferredGoal:
...
def _negation_judgment(goal: protomorph.Spec) -> urs.Judgment:
    rel = protomorph.Spec.of("std.logic.Not", goal)
...
def _coerce_bound_value(goal: CanonicalGoal, slot: int, value: Any) -> protomorph.Carrier:
    if isinstance(value, protomorph.Carrier):
        return value
    if isinstance(value, protomorph.Spec):
        return wrap_logic(value)
    return protomorph.LeafCarrier(goal.slots[slot].descriptor, value)


def _coerce_bound_value_for_carrier(slot: protomorph.Carrier, value: Any) -> protomorph.Carrier:
    if isinstance(value, protomorph.Carrier):
        return value
    if isinstance(value, protomorph.Spec):
        return wrap_logic(value)
    return protomorph.LeafCarrier(slot.descriptor, value)
...
def _matches_any_fact(goal: protomorph.Carrier, facts: set[protomorph.Spec] | tuple[protomorph.Spec, ...]) -> bool:
    for fact in facts:
        uf = make_union_find()
...
def _contains_runtime_vars(carrier: protomorph.Carrier) -> bool:
...
def _specs_from_carriers(carriers: tuple[protomorph.Carrier, ...], uf: UnionFind) -> tuple[protomorph.Spec, ...]:
    values: list[protomorph.Spec] = []
