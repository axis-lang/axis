...
class UnionFind:
    ...
    def bind(self, var: protomorph.Val, term: protomorph.Val, *, occurs_check: bool = True) -> bool:
        ...
        if rv is rt:
            return True
        if occurs_check and not self.is_var(rt) and len(rt.children) > 0:
...
def _walk(
    ...
    while stack:
        ...
        # both non-var
        left_is_leaf = len(left.children) == 0
        right_is_leaf = len(right.children) == 0
