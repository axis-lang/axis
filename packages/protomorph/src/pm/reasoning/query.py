...
class Query(Consed):
    ...
    @flux.property
    def query_placeholders(self) -> tuple[pm.Placeholder, ...]:
        _, placeholders, _ = instantiate_query(self.goal)
        return placeholders

    @flux.property
    def semantic_key(self) -> pm.Spec | None:
...
def _cycle_judgment(goal: pm.Spec, issue) -> urs.Judgment:
    ...
    if isinstance(issue, NegativeCycleIssue):
        ...
        evidence = pm.Spec.of("std.logic.ByMixedCycle", goal, trace if trace is not None else issue.cycle)
...
def _specialize_deferred(blocked, placeholders: tuple[pm.Placeholder, ...]):
    wakes = []
