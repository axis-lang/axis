...
class QueryCtx(ReasoningCtx):
    skeleton: pm.Spec
    public_placeholders: tuple[pm.Placeholder, ...] = ()
...
class ReasoningVar(pm.Var, abstract=True):
...
def source_name_of(var: pm.Var) -> str | None:
    source = getattr(var, "source_name", None)
    if callable(source):
        value = source()
        return value if isinstance(value, str) else None
    return pm.placeholder_name(var)


def origin_var_of(var: pm.Var) -> pm.Var:
