...
import pm
from pm import Builtin, Host, Spec, placeholder, spec_name
...
class SubstAnswerHost(Host):
    answer_placeholder: pm.Placeholder
...
class TestReasoningOperators(unittest.TestCase):
    ...
    def test_builtin_projection_returns_field_type(self):
        ...
        self.assertIsInstance(result, Unique)
        self.assertEqual(repr(cast(Unique, result).subst[r]), repr(Spec.of("Id")))
