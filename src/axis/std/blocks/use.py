...
class Destructuring(Object):
    ...
    def __call__(self, exp: syn.Expr, prefix: syn.Expr):
        ...
        def process_expr(elem): 
            ...
            if isinstance(elem, expr.Member):
                ...
                return expr.Tuple.NominalElement(
                    key=elem.as_sym(), 
                    value=elem,
                ).with_span_of(elem)
    ...
    @transform.register
    def eval_tuple_value_elem(self, elem: expr.Tuple.ValueElement, prefix: syn.Expr):
        #return elem.with_attrs(value=self.transform(elem.value, prefix))
        return self.transform(elem.value, prefix)

    @transform.register
    def eval_tuple_nominal_elem(self, elem: expr.Tuple.NominalElement, prefix: syn.Expr):
    ...
    @transform.register
    def eval_tuple_spread_elem(self, elem: expr.Tuple.SpreadElement, prefix: syn.Expr):
